"""Alembic environment, wired to linkedos settings and models.

The database URL comes from `Settings.db_path`, not from `alembic.ini`, so exactly one
place decides where the database lives and no connection string is ever committed.
`target_metadata` is the declarative `Base.metadata`, so `alembic revision
